...
public class toolsCombine : EditorWindow
{
    ...
    public static void ShowWindows()
    {
        ...
        wnd.titleContent = new GUIContent("工具合集");
    }
    ...
    public void OnEnable()
    {
        ...
        page_Index.Add(new Label("这是将各种工具整合的窗口"));
        ...
        page_QuickRename.Add(new Label("请选中要批量更改物体的父节点"));
        page_QuickRename.Add(new Label("一次请只选择一个"));
        page_QuickRename.Add(new Label("若选择多个将只操作第一个"));
        page_QuickRename.Add(new Label("------------------------"));
        Page_QuickName param = new Page_QuickName();

        var page_QuickRename_basename = new TextField("输入名字模板")
        {
            value = param.baseName,
            viewDataKey = "page_QuickRename_basename"
        };

        var page_QuickRename_isFGO = new Toggle("是否加FGO后缀")
        {
    ...
    class Page_QuickName
    {
        ...
        public void ReNameIt(Transform[] trans)
        {
            ...
            Debug.Log("命名模板：" + baseName);
            if (isFGO)
            {
                Debug.Log("将会添加_FGO后缀");
            }
            if (isFromOne)
            {
                Debug.Log("将会从1开始");
    ...
    class Page_LevelDugger
    {
        ...
        public void CreateUI(string n, VisualElement parent)
        {
            ...
            IntegerField text_levelNum = new IntegerField("关卡level");
            text_levelNum.viewDataKey = "page_LevelDebugger_level_num";
            IntegerField text_partNum = new IntegerField("关卡part");
