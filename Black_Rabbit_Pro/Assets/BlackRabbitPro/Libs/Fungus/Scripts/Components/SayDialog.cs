...
namespace Fungus
{
    ...
    public class SayDialog : MonoBehaviour
    {
        ...
        public static SayDialog GetSayDialog(string name)
        {
            Transform Dialog = GameObjectHelper.GetTransformByName(name);
            if (Dialog.GetComponent<SayDialog>() != null)
            {
                return Dialog.GetComponent<SayDialog>();
            }
            else
            {
                return null;
